...
usage = "ruby sam_simulator.rb\tdepth\tread_length\tchr\ttarget_pos\tvariants1\tvariants2\thaplotype_freq_tumor\thaplotype_freq_normal\tout_prefix
ex. ruby sam_simulator.rb\tdepth\tread_length\tchrA\t1000\t500,1500,2500\t750,1200,2600\t0.5,0.5,0.0,0.0\t0.25,0.5,0.25,0.0\t./out."
...
Settings.depth = ARGV[0].to_i
Settings.read_length = ARGV[1].to_i
chr = ARGV[2]
target_pos = ARGV[3].to_i
...
variants1_str = ARGV[4]
variants2_str = ARGV[5]
...
hap_freq_normal = ARGV[6].split(",").map {|x| x.to_f}
hap_freq_tumor = ARGV[7].split(",").map {|x| x.to_f}
